...
```
SPEC.md       # current-state design — editable
...
alternatives — do not re-propose them.
...
the leave window. Mitigated in §1.5, caught by the dead-man's switch regardless.
...
**Do not gitignore `data/`** — the SQLite file and JSON exports are the system's state by
design.
...
concurrency group prevents overlapping runs conflicting on a binary SQLite file.
...
Migrations as an ordered DDL list with a `schema_version` table. The schema *will* change
mid-build and you do not want to lose a crawl.
...
**Model:** Opus 5 · **Plan mode:** yes · **Est:** 4–6h · **Criteria:** C-0.1 – C-0.8
...
### M6 — Getro
**Model:** Sonnet 5 · **Plan mode:** no · **Est:** 2–4h · **Criteria:** C-6.1 – C-6.3

Both roles: discovery *and* the independent monitoring path that works without mapping.
...
SPEC §12.8 payload regime.

---

### M8 — Full discovery crawls
**Model:** Sonnet 5 · **Plan mode:** no · **Est:** 3–4h · **Criteria:** C-8.1 – C-8.4
...
```
M-1 → M0 → M1 → M2 → M3 → M4 → M10 → M8      (~27–35h)
```
...
- **SQLite + git.** The concurrency group from §1.4 prevents conflicts.
...
- **One integration test** running the pipeline against fixtures into a temp SQLite file.
