...
class TestFetchBoardCompanies:
    def test_a_successful_fetch_returns_parsed_companies(self):
        ...
        board = GetroBoard(name="Breakthrough Energy Ventures", url="https://breakthroughenergy.getro.com/jobs")
...
class TestDiscoverBoards:
    def test_c_6_1_two_real_boards_parse_and_their_companies_are_ingested_as_getro_sourced(self):
        ...
        def handler(request: httpx.Request) -> httpx.Response:
            if "breakthroughenergy" in str(request.url):
                return httpx.Response(200, content=read_fixture("breakthrough.html").encode())
            if "bluebearcap" in str(request.url):
                return httpx.Response(200, content=read_fixture("bluebear.html").encode())
            raise AssertionError(f"unexpected URL in test: {request.url}")

        boards = [
            GetroBoard(name="Breakthrough Energy Ventures", url="https://breakthroughenergy.getro.com/jobs"),
    ...
    def test_one_bad_board_does_not_stop_the_others_from_being_ingested(self):
        ...
        def handler(request: httpx.Request) -> httpx.Response:
            if "breakthroughenergy" in str(request.url):
                return httpx.Response(200, content=read_fixture("breakthrough.html").encode())
            return httpx.Response(200, content=read_fixture("no_next_data.html").encode())
    ...
    def test_re_running_discovery_creates_zero_duplicate_companies(self):
        ...
        boards = [GetroBoard(name="Breakthrough Energy Ventures", url="https://breakthroughenergy.getro.com/jobs")]
